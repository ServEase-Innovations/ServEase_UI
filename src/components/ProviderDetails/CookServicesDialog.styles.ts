...
export const TabButton = styled.button<{ active: boolean }>`
  ...
  font-weight: bold;
  cursor: pointer;
  color: ${props => props.active ? DINNER_COLOR : '#2d3436'};
...
export const TabIndicator = styled.div<{ active: boolean }>`
  border-bottom: ${props => props.active ? `2px solid ${DINNER_COLOR}` : 'none'};
  width: 50%;
  margin: 0 auto;
  padding-bottom: 8px; 
`;
...
// Additional Components (from the first code block)
