...
interface CustomerState {
  customerId: string | null; // Changed from number to string
  mobileNo: string | null;
  alternateNo: string | null;
  firstName: string | null; // Added for new response structure
  lastName: string | null;  // Added for new response structure
  emailId: string | null;   // Added for new response structure
...
export const fetchCustomerDetails = createAsyncThunk(
  ...
  async (customerId: string, { rejectWithValue }) => { // Changed parameter type to string
    try {
      const response = await providerInstance.get(
        `/api/customer/${customerId}` // Using the new API endpoint
      );
